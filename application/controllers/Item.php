...
class Item extends CI_Controller {
    ...
    public function edit($id)
    {
    	...
    	if ($query->num_rows() > 0) {
    		...
    		echo "<scripr>window.location='".base_url('item')."'</script>";
    	}
    }
	public function process()
	{
		...
		$config['file_name'] = 'item-'.date('ymd').'-'.substr(rand(),0,10);
			...
			if ($this->item_m->check_barcode($post['barcode'], $post['item_id'])->num_rows() > 0) {
				...
				if (@$_FILES['image']['name'] != null) {
					if ($this->upload->do_upload('image')) {
						...
						if ($item->image != null) {
							$target_file = './assets/uploads/product/'.$item->image;
							unlink($target_file);
						}
	...
	public function del($id)
	{
		...
		$target_qr = './assets/uploads/qr-code/item-'.md5($item->name).'.png';
		unlink($target_qr);
		$this->item_m->del($id);
		if ($this->db->affected_rows() > 0) {
